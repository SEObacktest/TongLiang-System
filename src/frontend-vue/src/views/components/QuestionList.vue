...
import {get_hr_list_request, get_question_list_request} from "@/api/api.js";
...
watch(
    () => props.user,
    (newVal, oldVal) => {
        refresh_question_list_all(newVal)
    }
)
...
async function refresh_activities_list(user) {
    ...
    question_list.value = await get_question_list_request({
    ...
    if(question_list.value.status == false) {
        alert(question_list.value.message)
    } else {
        question_list.value = question_list.value.data
    }
}
...
async function refresh_question_list_all(user) {
...
<div id='main'>
    ...
    <div class="activity-list">
        ...
        <div class="activity" v-for="question in question_list">
            ...
            <div class="hr-content-botton" @click="question_click(question)">查看详情</div>
        </div>
    </div>
</div>
